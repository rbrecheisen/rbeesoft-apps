...
class FileModel(BaseModel):
    ...
    name: Mapped[str] = Column('_name', String(256), nullable=False, unique=False)
    path: Mapped[str] = Column('_path', String(1024), nullable=False, unique=False)
    fileset: Mapped['FileSetModel'] = relationship('FileSetModel', back_populates='files')
    fileset_id: Mapped[str] = Column('_filesetmodel_id', ForeignKey('_filesetmodel._id', ondelete='CASCADE'), nullable=False, index=True)